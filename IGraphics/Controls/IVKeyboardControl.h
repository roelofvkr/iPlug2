...
class IVKeyboardControl : public IControl , public IMultiTouchControlBase
{
  ...
  public:
    ...
    void OnMouseDown(float x, float y, const IMouseMod& mod) override
    {
      ...
      mParent->AddTouch(mod.idx, x, y, mod.radius);
      mParent->SetHit(mod.idx, this);
    ...
    void OnMouseUp(float x, float y, const IMouseMod& mod) override
    {
      ...
      mParent->ReleaseTouch(mod.idx);
      mParent->ClearHitIfMovedOffkey(mod.idx, nullptr);
    }
    
    void OnMouseDrag(float x, float y, float dX, float dY, const IMouseMod& mod) override
    {
      mParent->UpdateTouch(mod.idx, x, y, mod.radius);
      mParent->HitMoved(mod.idx, this);
      
      SnapToMouse(x, y, EDirection::Vertical, mRECT, 1); // use val #1 for height
      mParent->SendCtrl1(mod.idx, GetValue(1));
    }
...
//    SetWantsMultiTouch(true);
   ...
   * @param idx The touch identifier
   * @param pKey ptr to the key being set on */
  void SetHit(uintptr_t idx, KeyControl* pKey)
  {
    if(pKey->GetValue() < 0.5)
    {
      pKey->SetValue(1.);
      pKey->SetDirty(false);
      mTouchPrevouslyHit[idx] = pKey;
   ...
   * @param idx The touch identifier
   * @param pKey ptr to the key being set on */
  void ClearHitIfMovedOffkey(uintptr_t idx, KeyControl* pKey)
  {
    auto itr = mTouchPrevouslyHit.find(idx);
   ...
   * @param idx The touch identifier
   * @param pKey ptr to the key being set on */
  void HitMoved(uintptr_t idx, KeyControl* pKey)
  {
    TrackedTouch* pTouch = GetTouchWithIdentifier(idx);
...
//    }
  ...
  void SendCtrl1(uintptr_t idx, double value)
  {
    auto touch = GetTouchWithIdentifier(idx);
